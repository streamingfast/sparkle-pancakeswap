...
mkdir ./localblocks
gsutil -m cp "gs://dfuseio-global-blocks-us/eth-bsc-mainnet/v1/000695*" ./localblocks/
...
function step1() {
    INFO=.* subgraph lightning step pancake/prediction/v1 -s 1 --output-path ./step1-v1 --start-block $1 --stop-block $2 --blocks-store-url $STOREURL &
}
function step2() {
    INFO=.* subgraph lightning step pancake/prediction/v1 -s 2 --input-path ./step1-v1 --output-path ./step2-v1 --start-block $1 --stop-block $2 --blocks-store-url $STOREURL &
}
function step3() {
    INFO=.* subgraph lightning step pancake/prediction/v1 -s 3 --input-path ./step2-v1 --output-path ./step3-v1 --start-block $1 --stop-block $2 --blocks-store-url $STOREURL &
}
function step4() {
    INFO=.* subgraph lightning step pancake/prediction/v1 -s 4 --flush-entities --store-snapshot=false --input-path ./step3-v1 --output-path ./step4-v1  --start-block $1 --stop-block $2  --blocks-store-url $STOREURL  &
}


main() {
  pushd "$ROOT" &> /dev/null
    go install -v ./cmd/subgraph || exit 1
...
#      step1 6952400 6952499
    ...
    if [ "$1" != "" ] && [ "$1" != csv ]; then
      ...
      INFO=.* prediction to-csv prediction --input-path ./step4-v1 --output-path ./stepcsv-v1 --chunk-size 1000
    fi
  popd &> /dev/null
}
